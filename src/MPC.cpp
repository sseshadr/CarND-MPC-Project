...
double act_weight = 5;
double delta_diff_weight = 200;
double a_diff_weight = 10;

// Create index positions for easy access to vars
size_t x_start = 0;
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // the Solver function below.
	  ...
	  // CALCULATE COST
	  // This holds the cost. Keep adding to this as a scalar.
	  fg[0] = 0;

	  // All costs are quadratic
	  ...
	  for (int t = 0; t < N - 1; t++) {
		  fg[0] += act_weight * CppAD::pow(vars[delta_start + t], 2);
		  fg[0] += act_weight * CppAD::pow(vars[a_start + t], 2);
	  }
	  ...
	  // SETUP CONSTRAINTS
	  // Since fg[0] belongs to cost, add 1 to index when assigning from vars to fg
